...
class CaptainController extends Controller {
    ...
    public function bouns($captainId) {
        try {
            $data = [
                'title' => 'Captain Bouns',
                'captain' => CaptainProfile::whereUuid($captainId)->with(['bounus', 'owner'])->orderBy('created_at', 'DESC')->first(),
            ];
            return view('dashboard.admin.captains.bouns', compact('data'));
        } catch (\Exception $e) {
            return redirect()->route('captains.index')->with('error', 'An error occurred while getting the captain details');
        }
    }

    public function updateBounsStatus(Request $request,$bounsId) {
        try {
            $this->captainService->updateBounsStatus($bounsId,$request->status);
            return redirect()->back()->with('success', 'captain bouns status updated successfully');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'An error occurred while updating the captain bouns status');
        }
    }
